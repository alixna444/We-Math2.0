...
class RayPPOTrainer:
    ...
    def fit(self):
        ...
        for _ in tqdm(range(self.config.trainer.total_epochs), desc="Epoch", position=0):
            for batch_tuple in tqdm(self.train_dataloader, desc="Running step", position=1):
                if isinstance(batch_tuple, tuple):
                    batch_dict, batch_indices = batch_tuple
                self.global_step += 1
                ...
                with timer("step", timing_raw):
                    ...
                    if batch_indices is not None:
                        difficulties = batch_dict["difficulty"]
                        
                    with timer("adv", timing_raw):
                        # get token level scores
                        reward_tensor, reward_metrics = ray.get(reward_ref)
                        batch.batch["token_level_scores"] = reward_tensor
                        scores = reward_metrics["accuracy"]
                        scores_grouped = [scores[i * self.config.worker.rollout.n: (i + 1) * self.config.worker.rollout.n] for i in range(self.config.data.rollout_batch_size)]
                        # pdb.set_trace()
                        self.train_dataloader.update_batch_results(batch_indices, scores_grouped, difficulties)
