            ...
            with timer("step", timing_raw):
                ...
                with timer("adv", timing_raw):
                    if "token_level_scores" not in batch.batch:
                        ...
                        """
                        Each 'id' uniquely identifies a sample.
                        We sort the 'rollout.n' scores for each sample, and rearrange all batch fields consistently, 
                        So the order is aligned across every field in the batch.
                        """

                        # Group indices by their unique sample id
                        id2indices: Dict[str, List[int]] = defaultdict(list)
                        for i, id_ in enumerate(batch.non_tensor_batch['id']):
                            id2indices[id_].append(i)

                        # Sort rollout scores within each id, record their new order of indices
                        id2sorted_indices = {}
                        for id_, indices in id2indices.items():
                            group_scores = scores[indices]
                            sorted_idx = group_scores.argsort(descending=True)
                            id2sorted_indices[id_] = [indices[i] for i in sorted_idx]


                        # Build the global new order for the batch based on the sorted rollouts within each problem (id)
                        new_order = []
                        for id_ in id2indices:
                            new_order.extend(id2sorted_indices[id_])

                        # Reorder all batch fields to align each sample's rollouts by their sorted position
                        for k, v in batch.batch.items():
                            batch.batch[k] = v[new_order]
                        for k, v in batch.non_tensor_batch.items():
                            batch.non_tensor_batch[k] = [v[i] for i in new_order]  

                        # Group indices by 'idx', which corresponds to same knowledge principle
                        idx2indices = defaultdict(list) 
                        for i, idx in enumerate(batch.non_tensor_batch['idx']):
                            idx2indices[idx].append(i)      

                        # For each knowledge principle group (idx),
                        # further divide by problem id, sort rollouts within each problem,
                        # then aggregate into a [num_problems, n_rollouts] matrix per group                        
                        idx2score_matrix = {}
                        for idx, indices in idx2indices.items():
                            id2indices_in_group = defaultdict(list)
                            for i in indices:
                                id_ = batch.non_tensor_batch['id'][i]
                                id2indices_in_group[id_].append(i)
                            group_score_matrix = []
                            for id_ in sorted(id2indices_in_group):
                                id_indices = id2indices_in_group[id_]
                                group_scores = scores[id_indices]
                                sorted_idx = group_scores.argsort(descending=True)
                                sorted_scores = group_scores[sorted_idx]
                                group_score_matrix.append(sorted_scores)
                            group_score_matrix = torch.stack(group_score_matrix)  # [num_problems, n_rollouts]
                            idx2score_matrix[idx] = group_score_matrix   

                        # For each group of the same knowledge principle,
                        # calculate the average reward at each sorted rollout position (across all problems in the group)
                        idx2mean_scores = {}
                        for idx, score_matrix in idx2score_matrix.items():                
                            mean_scores = score_matrix.mean(dim=0)  # Mean across problem dimension
                            idx2mean_scores[idx] = mean_scores

                        # Aggregate mean scores across all groups as part of the A_{i} calculation.
                        # So the original compute_advantage function is no longer called
                        mean_scores_list = []
                        group_size = self.config.worker.rollout.n
                        idx_tensor = batch.non_tensor_batch['idx']

                        num_groups = len(idx_tensor) // group_size

                        for group_idx in range(num_groups):
                            idx = idx_tensor[group_idx * group_size]
                            mean_scores_list.append(idx2mean_scores[idx])

                        metrics.update(reward_metrics)
                        mean_scores_tensor = torch.cat([t.flatten() for t in mean_scores_list])
                        bsz = scores.shape[0]
                        
                        id2score = defaultdict(list)
                        for i in range(bsz):
                            id2score[idx_tensor[i]].append(scores[i].item())

                        id2mean = {} # Group mean for normalization
                        id2std = {}  # Group std for normalization
                        for group_id in id2score:
                            vals = torch.tensor(id2score[group_id], device=mean_scores_tensor.device, dtype=mean_scores_tensor.dtype)
                            id2mean[group_id] = vals.mean()
                            id2std[group_id] = vals.std(unbiased=False)
                        normed_scores = torch.empty_like(mean_scores_tensor)
                        eps = 1e-6 # Avoid divide-by-zero
                        for i, idx in enumerate(batch.non_tensor_batch['idx']):
                            normed_scores[i] = (mean_scores_tensor[i] - id2mean[idx]) / (id2std[idx] + eps)

                        response_mask = batch.batch["response_mask"]
                        advantages = normed_scores.unsqueeze(-1) * response_mask
                        returns = advantages
